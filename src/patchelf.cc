        ...
        if (rdi(dyn->d_tag) == DT_NEEDED) {
            ...
            if (i != libs.end() && name != i->second) {
                ...
                if (a != addedStrings.end()) {
                    ...
                    /* This still mutates a DT_NEEDED entry, so mark the file
                       changed and flag a replacement; otherwise a stale
                       resolution-cache note could survive when every replace
                       reuses an earlier string (removeResolutionCache() below
                       is gated on `replaced`). */
                    changed = true;
                    replaced = true;
                    continue;
                }
                ...
                if (pos != std::string::npos && orig.size() == pos + replacement.size()) {
                    ...
                    /* ensure write-out in case all replaces use this technique;
                       flag a replacement too so a stale resolution-cache note is
                       dropped (removeResolutionCache() below is gated on
                       `replaced`). */
                    changed = true;
                    replaced = true;
                    continue;
...
template class ElfFile<Elf32_Ehdr, Elf32_Phdr, Elf32_Shdr, Elf32_Nhdr, Elf32_Addr, Elf32_Off, Elf32_Dyn, Elf32_Sym, Elf32_Versym, Elf32_Verdef, Elf32_Verdaux, Elf32_Verneed, Elf32_Vernaux, Elf32_Rel, Elf32_Rela, 32>;
template class ElfFile<Elf64_Ehdr, Elf64_Phdr, Elf64_Shdr, Elf64_Nhdr, Elf64_Addr, Elf64_Off, Elf64_Dyn, Elf64_Sym, Elf64_Versym, Elf64_Verdef, Elf64_Verdaux, Elf64_Verneed, Elf64_Vernaux, Elf64_Rel, Elf64_Rela, 64>;
